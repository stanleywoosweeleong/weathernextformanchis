...
locations around Manchis (文积), Pahang. Multilingual interface
...
This release migrates Manchis from the older forecast-only build onto
...
```
https://stanleywoosweeleong.github.io/weathernextformanchis/
```
...
|---|---|---|---|---|
| Manchis | 文积 | 3.20500, 102.16400 | open plain | durian |
| Kg. Ponson - Gan Ming Keong | 彭顺村 - 颜明强 | 3.229722, 102.156111 | sheltered basin | durian |
| Batu 36 | 三十六碑 | 3.240556, 102.141111 | sheltered basin | durian |
...
This build carries the seed version **`mc-arch1`** (the lowland-architecture
...
```
wnext-weathernextformanchis-202606081800
```
...
- **Storage namespace:** `weathernextformanchis__*` keys in `localStorage`, isolated
  from other WeatherNext regional builds so data never collides.
- **Cloud sync:** Firebase, namespaced under `appId: wnext-ag-v41-weathernextformanchis`.
...
The icon shows a graduate in an orchard under a blue sky. The boot screen
background (sky-blue `#9bd3f6`) is matched to the icon's sky.
